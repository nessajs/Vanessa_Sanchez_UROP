%GUI feeds in I_total, this computes current distribution at variable
%slices for the no defect case


%% Independent sources
runs = 5;
Slice_set = linspace(1,5,runs);
        ...
        if iter == 1
            ...
            for i = 1:tape_num % for each tape    
                ...
                for j = 1:slice_num % go through all the slices  
                        ...
                        % check if converged
                        if (abs((I_compute_old(i,j) - I_compute)/I_compute_old(i,j)) < 10^-3) | (iter == 500) %after finding that I = 200A does not converge with the given criteria, doing this
                            epsilons(i,j) = true;
                        end                
...
% for i = 1:5
%     rowdata = [];
%     for j = 1:length(converged)F
%         data = converged{j}(i); % Extract data from the ith cell
%         rowdata(end + 1) = data;
%     end
%     plot(1:length(converged), rowdata, 'o-');    % Plot the data
