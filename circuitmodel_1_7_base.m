...
%configuration. It produces graphs for the validation of the circuit solver
%and the convergence methods used. The netlist used is no_block_circuit.txt
...
fname = "C:\Users\Vaness Sanchez\Desktop\scam-master\no_block_circuit.txt"; %replace with path to .txt file
    ...
    if iter == 1
        ...
        for i = 1:tape_num % for each tape    
            ...
            for j = 1:slice_num % go through all the slices  
                    ...
                    % check if converged
                    if abs((I_compute_old(i,j) - I_compute)/I_compute_old(i,j)) < 10^-3
                        epsilons(i,j) = true;
                    end                
...
for row = 1:5
    ...
    for col = 1:5
     ...
     hold off;
     title(sprintf('Change in Tape %d I_{compute} Error Across Iterations', row_index));
%     xlabel('Iteration');
%     ylabel('Value');
     legend('Resistor 1', 'Resistor 2', 'Resistor 3', 'Resistor 4', 'Resistor 5');
end 
...
for row = 1:2
    ...
    hold off;
    title(sprintf('Current in Tape Former Across Iterations', row_index));
%    xlabel('Iteration');
%    ylabel('Value');
    legend('Resistor 1', 'Resistor 2', 'Resistor 3', 'Resistor 4', 'Resistor 5');
end
