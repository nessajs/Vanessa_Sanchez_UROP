%This script is the base circuit model for a 5 tape x 5 resistor
%configuration with a defect. It produces graphs for the validation of the circuit solver
%and the convergence methods used. The netlist used is no_block_defect_circuit.txt.
%which is an altered LTSpice netlist.
...
R_defect = R_inf;
R(3,3) = R_defect; %make it easier to change this, maybe don't hardcode the initialization of R
...
%for iter = 0:iterations %use line above instead if wanting to run until convergence
    ...
    if iter == 1
        ...
        for i = 1:tape_num % for each tape    
            ...
            for j = 1:slice_num % go through all the slices  
                    ...
                    % check if converged
                    if (abs((I_compute_old(i,j) - I_compute)/I_compute_old(i,j)) < 10^-3) | (iter == 500)| (R(i,j) == R_inf)
                        epsilons(i,j) = true;
                    end                
    ...
    %% current through tapes - will move this into the loop
%     I_tape = [];
%     for y_c = 1:N %tape num
%         tapeV = deltaVs(y_c,:);
%         I_set = [];
%         for x_c = 1:M %slice num
%             v = tapeV(x_c);
%             current = v/R(y_c,x_c);
%             I_set(end+1) = current;
%         end
...
%plot for I_compute and I_hts error method
figure(1);
clf;
for row = 1:5
    subplot(5,1,row);
    for col = 1:5
        row_index = row; %tape I want to look at

        %extract values for this column in the selected row across all slices
        slice_values = squeeze(E(row_index, col, :));
    
        %plot values across slices
        plot(1:iter, slice_values, 'o-'); 
        hold on; % Hold on to plot multiple lines on the same figure
    end

    hold off;
    title(sprintf('Change in Tape %d Error Across Iterations', row_index));
    xlabel('Iteration');
    ylabel('Value');
    legend('Resistor 1', 'Resistor 2', 'Resistor 3', 'Resistor 4', 'Resistor 5');
end  
        
%plot for I_compute error method
figure(2);
clf;
for row = 1:5
    subplot(5,1,row);
    for col = 1:5
        ...
        %extract values for this column in the selected row across all slices
        slice_values = squeeze(E_compute(row_index, col, :));
    ...
    hold off;
    title(sprintf('Tape %d - Change in I compute Error Across Iterations', row_index));
    xlabel('Iteration');
    ylabel('Value');
    legend('Resistor 1', 'Resistor 2', 'Resistor 3', 'Resistor 4', 'Resistor 5');
end 

% plot of current through tapes and former
figure(3);
clf;
for row = 1:5
    subplot(7,1,row);
    for col = 1:5
        ...
        %extract values for this column in the selected row across all slices
        slice_values = squeeze(I_compute_matrix(row_index, col, :));
    ...
    hold on;
    title(sprintf('Current in Tape %d Across Iterations', row_index));
    xlabel('Iteration');
    ylabel('Value');
    legend('Resistor 1', 'Resistor 2', 'Resistor 3', 'Resistor 4', 'Resistor 5');
...
for row = 1:2
    subplot(7,1,row+5);
    for col = 1:5
        row_index = row; %former side I want to look at

        %extract values for this column in the selected row across all slices
        slice_values = squeeze(former(row_index, col, :));
    ...
    for ix=1:nLines
        ...
        switch val
            case 'R_cu'
                val = R_Cu_val;
                Value{ix} = val;
            % replace R_defect in ref_value, now Value
